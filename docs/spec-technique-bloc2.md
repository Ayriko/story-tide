...
5. **Liaison automatique Aho-Corasick** (différenciateur) : détection serveur asynchrone, surlignage dans l'éditeur, relations `origin=AUTO`, mentions manuelles `@`, garde-fous (ignorer/délier une occurrence)
6. Backlinks sur chaque fiche
...
1. **Dictionnaire** = noms + alias de toutes les entités du monde (normalisés : casse, accents ; les deux formes conservées pour l'affichage).
2. **Automate compilé et mis en cache par monde** (Map en mémoire du worker + version en base) ; **invalidation** à toute création/renommage/suppression d'entité ou d'alias.
3. Déclenchement : sauvegarde de fiche → debounce → `queue.enqueue('link-scan', { entityId }, { singletonKey: entityId })`.
4. Scan du `plainText` : matches en un passage (O(n)), **plus long match prioritaire**, frontières de mots respectées, auto-mention exclue, `LinkIgnore` filtré.
5. Écriture : upsert des `Relation origin=AUTO` (diff : ajouts/suppressions), positions des occurrences renvoyées au client pour le **surlignage** (décorations ProseMirror — style distinct des mentions manuelles).
6. Homonymes : si deux entités matchent la même occurrence → pas de lien silencieux, marquage « ambigu » cliquable pour trancher.
7. Perf (parade risque principal C1.2.3) : automate caché + jobs async + index PG. Si goulet un jour : worker scalé indépendamment — jamais de microservices.
