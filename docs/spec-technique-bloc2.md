...
5. Écriture : upsert des `Relation origin=AUTO` (diff : ajouts/suppressions) — **jamais `MANUAL`, vérifié en conditions réelles**. **Fait.** Positions des occurrences renvoyées au client pour le **surlignage** (décorations ProseMirror) : **pas encore fait** (le worker écrit les relations mais ne renvoie rien au client pour l'instant — surlignage UI en attente).
