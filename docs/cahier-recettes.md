...
- **Type** : fonctionnel + accessibilité (clavier complet, RGAA) · **Statut** : ⏸️ Recette staging v1.2.0-rc.1 (2026-07-23) : bloqué par BUG-011 (aucune image ne s'affiche sur staging tant que l'URL présignée pointe vers l'hôte Docker interne) — nécessite une image visible pour tester le redimensionnement. **BUG-011 corrigé** (ADR-0023) — en attente de retest manuel sur staging (nouvelle rc). Gates automatisés historiques inchangés : ✅ (lint, `tsc`, 335/335 tests unitaires, build, 9/9 e2e ; validé manuellement par Aymeric).
