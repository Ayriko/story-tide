...
- **Objectif** : vérifier que le parcours est autonome de bout en bout (plus aucun geste de support manuel), que le message part réellement depuis `contact@storytide.fr`, et que l'ancien mot de passe cesse immédiatement de fonctionner.
- **Préconditions** : un compte existe avec une adresse e-mail réellement consultable ; les variables `SMTP_*`, `MAIL_FROM` et `MAIL_TRANSPORT=smtp` sont posées sur l'environnement testé.
- **Étapes** : 1) Sur `/login`, cliquer « Mot de passe oublié ? ». 2) Saisir l'adresse du compte, valider. 3) Ouvrir la boîte de réception, vérifier l'expéditeur affiché, ouvrir le lien du message. 4) Saisir un nouveau mot de passe (≥ 8 caractères) **et le confirmer dans le second champ**, en vérifiant au passage que la bascule œil affiche puis masque la saisie, valider. 5) Se connecter avec l'**ancien** mot de passe. 6) Se connecter avec le **nouveau**.
- **Résultat attendu** : 2) confirmation « Si un compte existe pour cette adresse, un e-mail vient d'être envoyé. », le formulaire disparaît ; 3) message reçu, expéditeur `contact@storytide.fr`, lien annonçant une validité d'une heure ; 4) redirection vers `/login` avec « Mot de passe modifié. » — **aucune session ouverte automatiquement** ; 5) échec avec « E-mail ou mot de passe incorrect. » ; 6) connexion réussie, arrivée sur `/worlds`.
- **Critères d'acceptation** : `e2e/password-reset.spec.ts` (parcours complet en navigateur réel, jeton lu dans la table `verification` puisque aucun message ne part en environnement de test) ; `src/actions/password-reset.test.ts` (jeton et mot de passe transmis à Better Auth, redirection `/login?reinitialise=1`, aucune connexion automatique) ; `reset-password-email.test.ts` (lien présent dans les deux formes du message, durée annoncée alignée sur `resetPasswordTokenExpiresIn`) ; envoi réel vérifié à la main avant livraison (l'expéditeur et la délivrabilité ne sont pas testables automatiquement).
- **Type** : fonctionnel (bout en bout) · **Statut** : ⬜ (e2e vert en local ; envoi réel et recette staging v1.3.1-rc.1 à passer).

## TST-AUT-011 — Réinitialisation : lien expiré, déjà utilisé ou invalide (KAN-52)

- **Description** : un lien de réinitialisation ne vaut qu'une fois et qu'une heure ; passé ce cadre, il est refusé et l'utilisateur se voit proposer d'en redemander un.
- **Objectif** : vérifier que le jeton est bien à usage unique et à durée limitée (un lien qui traîne dans une boîte ne doit pas rester une porte ouverte), et qu'un refus ne laisse jamais l'utilisateur sur une page sans issue.
- **Préconditions** : un compte existe ; un lien de réinitialisation a été demandé puis déjà utilisé une fois.
- **Étapes** : 0) Sur un lien **valide**, saisir deux mots de passe **différents** dans les deux champs et valider : l'erreur doit porter sur la confirmation et le lien doit rester utilisable ensuite. 1) Rouvrir le lien déjà consommé, saisir un mot de passe, valider. 2) Ouvrir `/reset-password` sans aucun paramètre `token`. 3) Ouvrir `/reset-password?token=jeton-fabrique-de-toutes-pieces`, saisir un mot de passe, valider. 4) Depuis l'un de ces écrans, cliquer « Demander un nouveau lien » et refaire une demande.
- **Résultat attendu** : 1) et 3) message « Ce lien de réinitialisation est invalide ou a expiré. », mot de passe **inchangé** ; 2) page « Lien expiré » sans formulaire ; dans les trois cas, un lien « Demander un nouveau lien » est présent ; 4) la nouvelle demande aboutit et le nouveau lien fonctionne. Le message est identique pour un jeton périmé, consommé ou fabriqué — il ne renseigne jamais sur la raison exacte du refus.
...
- **Préconditions** : `/login` accessible (aucune session active), artwork chargé (`public/artwork/login-hero-{1920,2880}.{avif,webp}` présents).
- **Étapes** : 1) Ouvrir `/login` à 1920×1080 (DPR 1) puis en DPR 2 (variante 2880). 2) Relever visuellement le titre, le sous-titre, l'onglet inactif, le pied de page. 3) Saisir un e-mail invalide et un mot de passe, soumettre — observer l'erreur de champ. 4) Soumettre des identifiants valides mais incorrects — observer l'alerte `role="alert"`. 5) Tabuler jusqu'au champ e-mail — observer l'anneau de focus. 6) Répéter 1-5 sur `/register`.
...
- **Type** : sécurité · **Statut** : ⚠️ Recette staging v1.2.0-rc.1 (2026-07-23) : volet faux MIME ✅ (rejet propre) ; volet taille >5 Mo ❌ — `500` brut. Aucune fuite de sécurité confirmée (rien en base ni MinIO). → `plan-correction-bogues.md` BUG-006, re-arbitré P1 le même jour après constat que la capacité spec (1-5 Mo) était aussi cassée, pas seulement le cas >5 Mo. **Corrigé** : `next.config.ts` (`bodySizeLimit: "6mb"`) + contrôle client immédiat (`checkImageFileSize`, `entity-editor.tsx`). Tests unitaires étendus : ✅ (`image-validation.test.ts` : 2 Mo/5 Mo acceptés, 6 Mo/5 Mo+1 rejetés ; `image-service.test.ts`). En attente de retest réel (upload 2 Mo/6 Mo) sur une nouvelle rc staging — le comportement `bodySizeLimit` du framework n'est pas vérifiable par test unitaire.
...
- **Type** : sécurité · **Statut** : ✅ Recette staging v1.2.0-rc.1 (2026-07-23) : premier passage en défaut (inspection Elements laissant croire à une balise `<a>` persistée), **corrigé après vérification empirique** — `$0.querySelector('a')` → `null`, `$0.outerHTML` → entités échappées, `$0.textContent` → texte littéral décodé. Le collage testé n'avait que `text/plain` sur le presse-papiers (fichier texte source, confirmé via écouteur `paste` en console) — intrinsèquement sûr, aucune balise réelle ne peut en résulter. Le cas `text/html` réel reste couvert par `tiptap-extensions.test.ts` (`generateJSON`, jamais modifié). `plan-correction-bogues.md` BUG-007 retiré (faux positif, panneau Elements mal interprété). Gates automatisés inchangés : ✅ (lint, `tsc`, 322/322 tests, build, 9/9 e2e).
...
## TST-SEC-017 — La demande de réinitialisation ne révèle pas l'existence d'un compte (KAN-52)

- **Description** : le formulaire « Mot de passe oublié » répond exactement la même chose que l'adresse saisie corresponde ou non à un compte existant — y compris lorsque l'envoi du message échoue côté serveur.
- **Objectif** : vérifier qu'aucun canal ne permet d'énumérer les comptes (OWASP A07) : ni le texte affiché, ni la présence d'une erreur, ni un comportement différent de la page. Un formulaire de réinitialisation est la cible classique de ce type d'inventaire.
- **Préconditions** : une adresse A rattachée à un compte existant, une adresse B qui n'a aucun compte, aucune session active.
- **Étapes** : 1) Demander une réinitialisation pour A, noter le message affiché et l'état de la page. 2) Recommencer avec B, comparer mot pour mot. 3) Vérifier qu'aucun message d'erreur n'apparaît dans le cas B. 4) Sur un environnement où l'envoi échoue (identifiants SMTP invalides), refaire une demande pour A et comparer à nouveau. 5) Inspecter les logs serveur du cas 4).
- **Résultat attendu** : dans tous les cas, le même texte « Si un compte existe pour cette adresse, un e-mail vient d'être envoyé. » en `role="status"`, le formulaire remplacé par cette confirmation, et **aucune** région `role="alert"`. Le cas 4) est indiscernable du cas nominal côté navigateur, alors que les logs serveur, eux, portent la cause réelle de l'échec d'envoi.
- **Critères d'acceptation** : `e2e/password-reset.spec.ts` (« la demande ne revele pas si une adresse a un compte » : adresse sans compte → même confirmation, zéro alerte) ; `src/actions/password-reset.test.ts` (retour strictement identique entre adresse connue et inconnue ; sur échec d'envoi, `sent: true` et `formError` absent, **et** `console.error` appelé avec la cause réelle) ; `forgot-password-form.test.tsx` (confirmation neutre, disparition du formulaire).
- **Type** : cas d'échec / sécurité · **Statut** : ⬜ (tests verts en local ; à passer à la recette staging de la 1.3.1-rc.1, cas 4) inclus).
...
- **Type** : fonctionnel (bout en bout) + accessibilité (alt obligatoire, RGAA) · **Statut** : ✅ Recette staging v1.2.0-rc.2 (2026-07-23) : premier passage en défaut sur rc.1 (bucket manquant BUG-010, puis affichage cassé BUG-011) — les deux corrigés (bucket automatisé, proxy image ADR-0023). Retest sur rc.2 : image visible immédiatement, `GET /api/media/<imageId>` répond `200` (plus `302`), persiste après rechargement. Capture prise.
...
- **Type** : fonctionnel + accessibilité (clavier complet, RGAA) · **Statut** : ✅ Recette staging v1.2.0-rc.2 (2026-07-23) : bloqué sur rc.1 par BUG-011 (image invisible), corrigé et retesté avec succès. Drag fluide confirmé, taille conservée après rechargement. Volet clavier confirmé après clarification : le clic sélectionne le nœud image (NodeSelection), le focus clavier doit ensuite atteindre la poignée `role="slider"` via **Tab** avant que les flèches gauche/droite agissent — une fois ce point clarifié, le réglage clavier fonctionne correctement, comportement standard non lié à un manquement RGAA.

## TST-ENT-013 — Édition d'une entrée dont le contenu dépasse la hauteur de l'écran (BUG-014, KAN-53)

- **Description** : une entrée dont le contenu (image insérée et/ou nombreux paragraphes) dépasse la hauteur du viewport reste entièrement utilisable : la saisie suit le curseur, l'en-tête et la barre d'outils restent atteignables, et le défilement s'arrête sur le pied de page sans zone vide au-delà. Une seule zone de la page défile — celle marquée `data-scroll-container` — jamais le shell qui la contient.
- **Objectif** : couvrir le cas « contenu > viewport », jamais exercé par la restructuration du conteneur défilant de KAN-45 (retours visuels et smoke sur pages courtes uniquement), et empêcher le retour de BUG-014 sous ses trois formes : « bloc » irrémovible en bas d'écran, texte saisi hors de vue, zone morte de la hauteur d'un écran sous le pied de page.
- **Préconditions** : un compte et un monde existent ; être sur la page d'une entrée.
- **Étapes** : 1) Insérer une image dans l'éditeur, puis cliquer un bouton de titre de la barre d'outils. 2) Saisir un contenu dépassant nettement la hauteur de l'écran (plusieurs dizaines de sauts de ligne), en continuant à taper une fois le bas atteint. 3) Remonter en haut de l'entrée. 4) Dérouler jusqu'en bas de la page. 5) Sur une page **courte** (`/worlds`), vérifier que le pied de page reste hors du premier écran et n'apparaît qu'au chevron.
...
## TST-ENT-017 — Bascule Dossiers/Types et arborescence accessible dans le panneau de gauche (KAN-57)

- **Description** : le panneau de gauche propose désormais deux vues exclusives pour parcourir les entrées d'un monde — « Types » (regroupement existant, inchangé) et « Dossiers » (nouvel arbre pliable branché sur la couche service de KAN-38). Aucune création de dossier dans ce lot : l'arborescence existante s'affiche telle quelle, les entrées sans dossier apparaissent sous « Non classé ».
- **Objectif** : vérifier que la bascule change effectivement l'affichage sans jamais modifier le comportement existant de la vue Types (y compris pendant une recherche), que le choix de vue survit au rechargement, et que l'arbre se navigue intégralement au clavier (rôles ARIA, tabindex flottant, flèches, Début/Fin) sans reposer sur la couleur seule.
- **Préconditions** : un compte avec au moins un monde contenant des entrées ; aucun dossier créé (hors périmètre) — seul « Non classé » est peuplé.
- **Étapes** : 1) Ouvrir un monde, observer la vue par défaut. 2) Cliquer la bascule « Dossiers ». 3) Recharger la page. 4) Taper une recherche en vue Dossiers, puis en vue Types. 5) Revenir en vue Dossiers hors recherche, déplier « Non classé » au clavier (flèche droite), naviguer (flèches haut/bas, Début/Fin), replier (flèche gauche).
- **Résultat attendu** : 1) vue « Types » active par défaut (regroupement par type, inchangé) ; 2) la vue « Dossiers » affiche l'arbre (« Non classé » présent, replié) ; 3) le choix « Dossiers » est toujours actif après rechargement ; 4) en Dossiers la recherche affiche une liste plate sans en-tête, en Types le regroupement avec en-têtes reste identique à avant ce lot ; 5) navigation clavier complète, un seul élément focusable (`tabIndex=0`) à la fois, anneau de focus visible sur le premier et le dernier élément.
- **Critères d'acceptation** : `folder-tree-utils.test.ts` (construction de l'arbre, aplatissement expansion-aware) ; `folder-tree.test.tsx` (rôles `tree`/`treeitem`/`group`, navigation clavier complète, invariant tabindex flottant vérifié à plusieurs étapes) ; `entity-search.test.tsx` (bascule effective, non-régression de la vue Types pendant une recherche, liste plate en Dossiers) ; vérification manuelle en navigateur (anneau de focus premier/dernier élément, contraste).
- **Type** : fonctionnel + accessibilité · **Statut** : ✅ (`folder-tree-utils.test.ts`, `folder-tree.test.tsx`, `entity-search.test.tsx` — 24 tests unitaires verts ; validation manuelle en navigateur, 2026-09-11) — ⬜ à repasser à la recette staging.

## TST-ENT-018 — CRUD des dossiers et déplacement d'une entrée, glisser-déposer et menu clavier « Déplacer vers... » (KAN-60)

- **Description** : ferme la boucle laissée ouverte par KAN-57 — créer, renommer et supprimer un dossier depuis la vue Dossiers, et déplacer une entrée dans ou hors d'un dossier. Deux mécanismes équivalents : glisser-déposer à la souris (confort, `@dnd-kit/core`, capteur pointeur seul) et menu « Déplacer vers... » sur chaque entrée (mécanisme principal, accessible par défaut - WCAG 2.5.7 « Dragging Movements » exige qu'une action déclenchable par glisser le soit aussi sans glisser).
- **Objectif** : vérifier que le CRUD des dossiers fonctionne et respecte le vrai comportement de suppression (sous-dossiers supprimés, entrées jamais détruites, repassées « Non classé ») ; que le glisser-déposer déplace réellement une entrée et que ce déplacement persiste après un rechargement complet (preuve d'écriture en base, pas seulement d'un état optimiste côté client) ; que le menu clavier couvre à lui seul la totalité de la fonctionnalité, sans souris.
- **Préconditions** : un compte avec un monde contenant au moins une entrée ; vue Dossiers active.
- **Étapes** : 1) Créer un dossier depuis « + Nouveau dossier ». 2) Déplier « Non classé », glisser une entrée sur le dossier créé (séquence de mouvements réels du pointeur). 3) Déplier le dossier, vérifier la présence de l'entrée. 4) Recharger la page entièrement, re-déplier le dossier. 5) Ressortir l'entrée via le menu « Déplacer vers... » (Tab jusqu'au déclencheur, Entrée, choix « Non classé ») sans aucune interaction souris. 6) Renommer le dossier. 7) Le supprimer (confirmation explicite) et vérifier que l'entrée (jamais assignée à ce dossier dans ce scénario) reste intacte dans « Non classé ».
- **Résultat attendu** : 1) le dossier apparaît dans l'arbre ; 2)-3) l'entrée quitte « Non classé » et apparaît sous le dossier cible ; 4) toujours présente après rechargement complet (persistance réelle, pas une illusion optimiste) ; 5) le menu clavier ressort l'entrée sans aucun geste de glisser ; 6) le nouveau nom s'affiche ; 7) le dossier disparaît, l'entrée reste consultable, jamais supprimée.
- **Critères d'acceptation** : `entity-service.test.ts` (`moveEntityToFolder` — autorisation, dossier d'un autre monde, `folderId: null`) ; `folder-service.test.ts` (`getFolderTree` inclut les entités d'un dossier) ; `folder-tree-utils.test.ts` (fusion sous-dossiers/entités, `flattenFolderOptions`, `moveEntityInTree`) ; `folder.test.ts` (CRUD des trois actions) ; `folder-tree.test.tsx` (bouton d'actions par dossier, menu « Déplacer vers... » opérable au clavier) ; `create-folder-form.test.tsx` (fermeture du dialogue après succès, sans navigation) ; `e2e/folder-organization.spec.ts` (glisser-déposer réel + persistance après rechargement + CRUD complet, vrai navigateur Chromium, vraie base Postgres isolée — le glisser physique du pointeur n'est testable qu'ainsi, jamais en jsdom).
- **Type** : fonctionnel + accessibilité + sécurité (autorisation `moveEntityToFolder`, cf. `docs/securite-owasp.md` A01) · **Statut** : ✅ (tests unitaires + `e2e/folder-organization.spec.ts`, 2 scénarios verts, 2026-09-11) — ⬜ à repasser à la recette staging.
