...
- **Description** : l'artwork de marque (`--bg-image`, `(auth)/layout.tsx`) est branché en fond plein écran de `/login` et `/register`, sans le voile de protection utilisé pour un artwork de monde quelconque (retiré, ADR-0025) — la lisibilité du panneau et du pied de page repose sur leurs propres calques (`bg-card/55` + `backdrop-blur-xl`, `bg-black/80`).
...
- **Critères d'acceptation** : mesuré sur le rendu réel (méthode détaillée dans `docs/accessibilite-rgaa.md`, capture + texte transparent + calcul WCAG 2.1) : titre, sous-titre, onglet, erreur de champ et pied de page ≥ 4,5:1 en DPR 1 et DPR 2 — **sauf** le sous-titre en variante 2880/DPR 2 (3,55:1, écart connu et assumé, voir ADR-0025). Anneau de focus vérifié visuellement (méthode automatisée jugée non fiable, l'anneau contamine son propre échantillon de mesure).
- **Type** : accessibilité · **Statut** : ⬜ (mesure automatisée faite cette session, cf. ci-dessus — passage manuel Aymeric et recette staging restent à faire).
